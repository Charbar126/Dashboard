defmodule Dashboard.SpotifyTokensTest do
  ...
  describe "spotify_tokens" do
    ...
    test "create_spotify_token/1 with valid data creates a spotify_token" do
      valid_attrs = %{access_token: "some access_token", refresh_token: "some refresh_token", expires_at: ~U[2025-02-07 23:26:00Z]}

      assert {:ok, %SpotifyToken{} = spotify_token} = SpotifyTokens.create_spotify_token(valid_attrs)
    ...
    test "update_spotify_token/2 with valid data updates the spotify_token" do
      spotify_token = spotify_token_fixture()
      update_attrs = %{access_token: "some updated access_token", refresh_token: "some updated refresh_token", expires_at: ~U[2025-02-08 23:26:00Z]}

      assert {:ok, %SpotifyToken{} = spotify_token} = SpotifyTokens.update_spotify_token(spotify_token, update_attrs)
    ...
    test "update_spotify_token/2 with invalid data returns error changeset" do
      spotify_token = spotify_token_fixture()
      assert {:error, %Ecto.Changeset{}} = SpotifyTokens.update_spotify_token(spotify_token, @invalid_attrs)
    ...
    test "delete_spotify_token/1 deletes the spotify_token" do
      ...
      assert_raise Ecto.NoResultsError, fn -> SpotifyTokens.get_spotify_token!(spotify_token.id) end
    end
